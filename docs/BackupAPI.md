...
# **getBackupsCounts**
```swift
    open class func getBackupsCounts(from: Date? = nil, to: Date? = nil, paid: Bool? = nil, completion: @escaping (_ data: BackupsCountResultWrapper?, _ error: Error?) -> Void)
```
...
BackupAPIApi.getBackupsCounts(from: from, to: to, paid: paid) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
